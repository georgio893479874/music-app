...
const ArtistPage = () => {
  ...
  useEffect(() => {
    const fetchArtistData = async () => {
      try {
        const { data: artistData } = await axios.get(`http://localhost:4521/performer/${artistId}`);
        setArtist(artistData);

        const { data: albumData } = await axios.get(`http://localhost:4521/album?artistId=${artistId}`);
        setAlbums(albumData);
      } catch (error) {
        console.error(error);
      }
  ...
  const handlePlay = async () => {
    try {
      const { data: songs } = await axios.get(`http://localhost:4521/track?artistId=${artistId}`);
      if (songs.length > 0) {
        setPlaying(true);
      }
    } catch (error) {
      ...
      <div className="flex-1 lg:ml-64">
        <div className="relative w-full h-[400px] overflow-hidden">
          <img
            ...
            alt={artist.name}
          />
          <button className="absolute top-4 right-4 text-white bg-black/50 p-2 rounded-full hover:bg-black/70 transition">
            <BsThreeDots size={18} />
          </button>
              ...
              <Link href={`/album/${album.id}`} key={album.id} passHref>
                <div className="relative group cursor-pointer">
                  <div className="relative overflow-hidden rounded-lg">
                    <img className="w-full h-auto object-cover transition duration-300 group-hover:brightness-50" src={album.coverUrl} alt={album.title} />
                  ...
                  <h3 className="mt-2 text-xl font-semibold transition duration-300 group-hover:underline">{album.title}</h3>
                  <p className="text-gray-400 text-sm">{new Date(album.releaseDate).getFullYear()}</p>
                </div>
              </Link>
            ))}
          </div>
