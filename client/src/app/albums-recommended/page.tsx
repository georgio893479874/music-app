...
export default function AlbumsRecommendedPage() {
  ...
  useEffect(() => {
    async function fetchAlbums() {
      ...
      const res = await axios.get<Album[]>(`${API_URL}/recommendation/user/${userId}/recommended-albums`)
      setAlbums(res.data)
      setLoading(false)
    }
    fetchAlbums()
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '32px auto', padding: 32 }}>
      <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 18 }}>Recommended Albums</h1>
      {loading ? "Loading..." : (
