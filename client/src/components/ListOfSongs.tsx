      ...
      <div className="min-h-screen bg-gradient-to-b from-neutral-900 to-neutral-800 flex flex-col lg: ml-60">
        <div className="flex flex-col md:flex-row items-center md:items-end gap-8 px-8 pt-16 pb-8">
          ...
          <div className="flex flex-col gap-4">
            <span className="uppercase text-xs tracking-widest text-gray-400 font-semibold">{label}</span>
            ...
            <div className="flex items-center gap-4 mt-2">
              {showFavoriteButton && onToggleFavorite && (
                <button
                  ...
                  className={`p-2 rounded-full border border-gray-600 hover:bg-gray-700 transition ${isFavorite ? 'text-red-500' : 'text-gray-300'}`}
          ...
          <table className="w-full text-left rounded-lg overflow-hidden bg-neutral-900 shadow-lg">
            ...
            <tbody>
              {tracks.map((track, index) => (
                <tr key={track.id} className="border-b border-gray-800 hover:bg-neutral-800 transition">
                  <td className="py-2 px-4 text-gray-400">{index + 1}</td>
                  <td className="py-2 px-4 text-white">{track.title}</td>
                  <td className="py-2 px-4 text-gray-300">{track.album?.artist?.name || track.album?.artistName || '-'}</td>
                  <td className="py-2 px-4 text-gray-300">{track.album?.title || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
